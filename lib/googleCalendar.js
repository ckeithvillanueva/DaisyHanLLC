...
async function createBookingEvent({ summary, startTime, endTime, userEmail, description }) {
  ...
  const event = {
    summary,
    description,
    start: { dateTime: startTime }, // UTC ISO string — no hardcoded timeZone
    end:   { dateTime: endTime },
    // Adding the client as an attendee makes Google send them a proper
    // calendar invitation email with the Google Meet link.
  ...
  const response = await calendar.events.insert({
    ...
    // 'all' → Google sends a calendar invite email to every attendee.
    sendUpdates: "all",
  });
