...
async function createBookingEvent({ summary, startTime, endTime, userEmail, description }) {
  ...
  // Try to extract Meet link from insert response.
  // Service accounts on personal Google accounts often don't get one,
  // so fall back to fetching the event, then to a standing link env var.
  ...
  if (!meetLink) {
    try {
      ...
      meetLink = fetched.data?.conferenceData?.entryPoints?.find(
        ep => ep.entryPointType === "video"
      )?.uri || null;
    } catch (_) { /* ignore */ }
  }
  ...
  if (!meetLink) {
    meetLink = process.env.GOOGLE_MEET_LINK || null;
  }
...
async function sendConfirmationEmail({
  userEmail, userName, startTime, endTime, topic, session, meetLink, timeZone,
}) {
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:'Helvetica Neue',Arial,sans-serif;">
  <div style="max-width:600px;margin:40px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(26,23,20,.08);">
    ...
    <div style="padding:40px;">
      ...
      <p style="font-size:16px;color:#4A4540;margin:0 0 32px;line-height:1.75;">
        ...
        ${meetLink ? "Your Google Meet link is below — save it to join the call at the scheduled time." : "Daisy will follow up with a meeting link before your session."}
      </p>
      ...
      <div style="text-align:center;margin-bottom:32px;">
        <a href="${meetLink}" style="display:inline-block;background:#28A880;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:14px 32px;border-radius:8px;">
          Join Google Meet →
        </a>
        <p style="font-size:12px;color:#7A746E;margin:10px 0 0;">${meetLink}</p>
      </div>` : ""}
      ...
      <ul style="font-size:15px;color:#4A4540;line-height:1.85;padding-left:20px;margin:0 0 32px;">
        <li style="margin-bottom:6px;"><strong>Save the Google Meet link above</strong> — you'll use it to join the call.</li>
  ...
  // Notify Daisy of the new booking.
  const DAISY_EMAIL = process.env.DAISY_EMAIL || GMAIL_USER;
  await transporter.sendMail({
    from:    `"Daisy Han Consulting" <${GMAIL_USER}>`,
    to:      DAISY_EMAIL,
    replyTo: userEmail,
    subject: `New booking: ${userName} — ${sessionLabel} on ${dateStr}`,
    html: `<p style="font-family:Arial,sans-serif;font-size:15px;color:#1A1714;">
      <strong>New booking received</strong><br><br>
      <strong>Client:</strong> ${userName} (${userEmail})<br>
      <strong>Session:</strong> ${sessionLabel}<br>
      <strong>Date:</strong> ${dateStr}<br>
      <strong>Time:</strong> ${timeRange}<br>
      ${topic ? `<strong>Topic:</strong> ${topic}<br>` : ""}
      ${meetLink ? `<strong>Meet link:</strong> <a href="${meetLink}">${meetLink}</a><br>` : ""}
    </p>`,
  });

  console.log(`Notification email sent to ${DAISY_EMAIL}`);
}
